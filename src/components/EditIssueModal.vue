...
import type { Task } from '../stores/issueStore'
...
onMounted(() => {
  editedIssue.value = {
    ...
    reporter: props.task.reporter,
    assignees: props.task.assignees.join(', '),
  }
})

const handleSubmit = () => {
  const assigneesArray = editedIssue.value.assignees
      .split(',')
      .map((a) => a.trim())
      .filter((a) => a)

  issueStore.updateTask({
    id: props.task.id,
    columnId: props.columnId.toString(),
    updatedTask: {
      ...props.task,
      type: editedIssue.value.type,
      title: editedIssue.value.title,
      description: editedIssue.value.description,
      priority: editedIssue.value.priority,
      reporter: editedIssue.value.reporter,
